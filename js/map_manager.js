...
//           }
          ...
          if (typeof(_features)!="undefined" && _features.length > 0) {
            ...
            if (_features.length>1){
              ...
              // <a href='javascript:table_manager.generate_table(map_manager.last_click_features);table_manager.page_start="+_features.length+";table_manager.show_total_records(map_manager.last_click_features.length);$(\"#data_table_spinner\").hide();$(\"#advanced_table_filters\").hide();'>"+LANG.IDENTIFY.SHOW_IN_TABLE+"</a>
...
//            }
       ...
       show_popup_details_show_num(num){
        console.log("show_popup_details_show_num",num)
        if (!num){
            // default setting
            this.result_num=0
        }else{
        ...
        $("#props_table").html(html)
         $("#details_but").html("<a href='javascript:filter_manager.show_details(\""+this.features[this.result_num].properties._id+"\")'>Show Details</a>")
        // update the text
    ...
    show_highlight_geo_json(geo_json){
        ...
        }else if (typeof(geo_json)!="undefined"){
             geo_json =geo_json.toGeoJSON()
            this.highlighted_feature =  L.geoJSON(geo_json,{
